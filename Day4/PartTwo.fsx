...
let tryLastWinningBoard bingo =
    let rec imp acc boards numbers =
        match numbers with
        | n :: ns ->
            let marked = List.map (mark n) boards
            let winningBoard =
                marked
                |> List.filter (fun b -> hasMarkedRow b || hasMarkedColumn b)

            match winningBoard with
            | [] -> imp acc marked ns
            | bs ->
                let newAcc =
                    (List.map (fun b -> n, b) bs) @ acc

                let newBoard = marked |> List.except bs
                imp newAcc newBoard ns
        | [] -> acc
    
    imp [] bingo.Boards bingo.Numbers
    |> List.tryHead

fsi.AddPrinter<Board>(fun (Board m) ->
    "\n"
    + String.concat "\n" [
        let rs =
            m
        ...
        for xs in rs do
            let columns =
                xs
                |> Seq.sortBy (fun (pos, _) -> pos.X)
                |> Seq.map snd

            String.concat "" [
                for cell in columns do
                    match cell with
                    | Number n -> sprintf "%3i" n
                    | Marked -> "  X"
            ]
    ]
    + "\n")
