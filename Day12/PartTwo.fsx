...
let isVistedTwice list =
    let rec imp acc list =
        match list with
        | [] -> false
        | (Small _) as x :: xs when List.contains x xs -> true
        | (Small _) as x :: xs -> imp (x :: acc) xs
        | _ :: xs -> imp acc xs
...
let rec findPath { Current = current; Visited = visited; Remaining = remaining } =
    ...
    | Some nexts ->
        let newRemaining =
            match current with
            | Small _ when List.contains current visited ->
                current :: (List.choose (function Small _ as x -> Some x | _ -> None) visited)
                |> List.fold (fun acc p ->
                    acc
                    |> Map.remove p
                    |> Map.map (fun _ ps -> ps |> List.filter ((<>) p))) remaining
            | Small _ when isVistedTwice visited ->
                remaining
                |> Map.remove current
